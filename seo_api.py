...
class ProductInput(BaseModel):
    ...
    name: str = Field(..., example="ASUS TUF Gaming F15 FX507 Intel Core i7 RTX 3050")
    short_description: Optional[str] = Field(
        None,
        example="ASUS TUF gaming notebook with strong performance and durability."
    )
    description: Optional[str] = Field(
        None,
        example="Equipped with Intel Core i7, RTX 3050, 16GB RAM and 144 Hz display."
    )
...
class SkuInput(BaseModel):
    """Input model to generate SEO starting from a SKU in ASUS BR staging."""
    sku: str = Field(..., example="90NR0GW3-M00B20")
    language: str = Field(
        "pt-BR",
        example="pt-BR",
...
def fetch_product_from_asus_br_staging(sku: str) -> dict:
    """
...
def generate_seo_fallback(product: ProductInput) -> SeoMetaOutput:
    ...
    """
    name = product.name.strip() or "Produto ASUS"

    country_label = "Brasil" if product.country.upper() == "BR" else product.country

    # Meta title
    raw_title = f"{name} | ASUS {country_label}"
    ...
    if lang.startswith("pt"):
        raw_description = (
            f"{name} com desempenho confiável para o dia a dia. "
            f"Veja mais detalhes na loja oficial ASUS {country_label}."
        )
    elif lang.startswith("es"):
        raw_description = (
            f"{name} con rendimiento confiable para el día a día. "
            f"Conoce más en la tienda oficial ASUS {country_label}."
        )
    else:
        raw_description = (
            f"{name} with reliable performance for everyday use. "
            f"Learn more at the official ASUS store in {country_label}."
        )
    ...
    words = [
        ...
        if len(w) > 3 and w.lower() not in {"asus", "gaming", "notebook"}
    ]
...
async def generate_seo_meta_from_sku(sku_input: SkuInput) -> SeoMetaOutput:
    ...
    """
    try:
        raw_product = fetch_product_from_asus_br_staging(sku_input.sku)
...
async def test_product_lookup(sku: str):
    ...
    """
    try:
        data = fetch_product_from_asus_br_staging(sku)
        return data
    except Exception as exc:
